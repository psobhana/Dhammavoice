...
def replace_text_in_line(line, replacement_text):
    # Use regular expression to find [space]( and ) part
    match = re.search(r'( \()(.*?)(\))', line)
    if match:
        # Construct the new line with the replacement text and leave the rest unchanged
        return line[:match.start(2)] + replacement_text + line[match.end(2):]
...
for i in range(len(xml_content)):
    # Ignore lines before finding [space](.
    if " (" in xml_content[i]:
        # Replace the text if the line contains [space](.
